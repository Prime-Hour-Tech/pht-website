...
- **Run `pnpm seed` to populate Sanity dataset (Slice 17 HANDOFF).** Comprehensive seed file at `apps/studio/seed/initial-content.ndjson` covers all 25 docs needed for the build to succeed and routes to render: 7 page docs (home, about, industries, contact, services, blog, switching) with full block sequences, 6 services, 4 team members (Devin + 3 placeholders), 3 starter posts, 2 example landing pages, termsPage + privacyPage with full Portable Text bodies. From `apps/studio/`, run `pnpm seed`. See `apps/studio/seed/README.md` for caveats (image uploads still separate; `--replace` flag if overwriting existing docs; `contactInfo` re-save requires `--replace`). After seeding, replace placeholder content in Studio: real engineer names/bios for the 3 placeholder teamMembers, real customer testimonials, swap design's placeholder phone/email/address in `contactInfo` to actual values.
- **Upload images in Studio after seeding (Slice 17 HANDOFF, image subset).** The seed leaves all image fields empty. Upload in Studio after `pnpm seed`:
  - `siteSettings.logoDark` + `siteSettings.logoLight` (PHT logo variants)
  - `siteSettings.defaultOgImage` (1200×630 PHT-branded share card)
